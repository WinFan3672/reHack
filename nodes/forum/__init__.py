...
chan_isp = chan_general.add_topic("facetious", "Super Scummy ISPs", """Have any of you visited the WarpMedia website?
...
chan_isp.reply("admin", "interesting observation")
chan_isp.reply("buffer_overflow", "Yeah they literally have over a dozen asterisks attached to their homepage alone")
chan_isp.reply("silent", "they're just as scummy as ConnectCast, if not worse")
chan_isp.reply("admin", "Half of the SuperBoost plans are more expensive than using a 'normal' plan (The first SuperBoost plan is £15 more expensive than the second normal one)")
chan_isp.reply("bit", "Oh, I never noticed that! Good thing I don't live in the UK!")
chan_isp.reply("vc", "They also claim unlimited data with SuperBoost, but the fine print says the first 250GB is unlimited")
chan_isp.reply("vc", "Meaning it's the same as the quote-unquote normal plans!")
chan_isp.reply("admin", "With a 10gbps plan you'd use up 250GB in less than a minute, then they'd throttle you down to 1Gbps")
chan_isp.reply("silent", "wrong: they remove the SuperBoost speeds when you get tiered down, but you continue to pay full price")
chan_isp.reply("vc", "Oh, £500 a month for less than gigabit? Thanks a LOT")
chan_isp.reply("facetious", "Rumour has it that a class-action is a-brewing")
chan_isp.reply("admin", "I would love for all 35 million households they sell their garbage to to have their day in court")
chan_isp.reply("facetious", "is it ok if I cross-post this to n/funny?")
chan_isp.reply("admin", "For sure, more people need to hear about it. I'd post in n/technology though, since it's not that funny")
chan_isp.reply("nerd", "since someone mentioned nerdnet, i might as well chime in")
chan_isp.reply("nerd", "as much as I'd love for my site to grow, I don't think it will")
chan_isp.reply("nerd", "I'm using standard forum software in a fairly nonstandard way")
chan_isp.reply("nerd", "I plan to introduce a ranking system, where users can up-rank good posts and replies and down-rank bad ones")
chan_isp.reply("nerd", "Then their account gains +0.25 rankpoints per up-rank and -0.25 per down-rank")
chan_isp.reply("nerd", "Once your has a certain amount of rankpoints, your account gets Verified, and shows up with a badge after every post")
chan_isp.reply("vc", "That feature would be AWESOME, but it would require custom server software")
chan_isp.reply("nerd", "That's what scares me: i cant program and i have no $$ to hire someone")
chan_isp.reply("rehack", "Perhaps consider finding a way to make money, or just get volunteers")
chan_isp.reply("admin", "I'm sure I could start a donation drive?")
chan_isp.reply("nerd", ">>> I'm sure I could start a donation drive?\n\nThanks for the offer, but that seems wrong to me")
chan_isp.reply("admin", "I could handle the hiring from the cash directly, so that I am the one accountable?")
chan_isp.reply("nerd", "All right, fine, ya twisted me arm")
